...
- **LFO 4개**: 6가지 파형(사인·삼각·업/다운 톱니·사각·S&H), 딜레이·라이즈,
  Trigger/Free/Envelope 트리거 모드, 호스트 BPM 동기(1/128 ~ 4Bar, 셋잇단·점음표)
...
LFO 파형은 LFO 패널의 웨이브 창을 클릭하면 순서대로 바뀝니다. 디자인의 LFO 패널에는
긴 선택 박스가 하나뿐이고 그 라벨이 TRIG이라, 디자인에 없는 박스를 새로 만드는 대신
창 자체를 파형 컨트롤로 썼습니다.

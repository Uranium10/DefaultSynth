...
- **모듈레이션 매트릭스 8슬롯**: 소스 11종(LFO 1–4 / 앰프·필터·모드 엔벨로프 /
  벨로시티 / 키트랙 / 모드휠), 데스티네이션 19종(피치·필터 A/B 컷오프·레조넌스·
  음량·팬·OSC A/B/C의 워프·레벨·디튠·팬), 바이폴라 양 조절
- **드래그로 연결**: ENV/LFO 탭의 ✛를 노브 고리에 끌어다 놓으면 라우팅이 생깁니다
...
디자인대로, ENV 1–3과 LFO 1–4 탭 오른쪽 그리고 VOICING의 NOTE/VELO 위에 ✛ 핸들이
있습니다. 이걸 눌러 끌고 가서 노브 왼쪽 위의 작은 고리에 놓으면 그 소스가 그 노브에
연결됩니다.

| 조작 | 결과 |
| --- | --- |
| ✛를 노브 고리로 드래그 | 매트릭스 빈 슬롯에 연결(양 50%) |
| 고리 우클릭 | 그 목적지로 가는 연결을 모두 해제 |

고리가 회색이면 연결 가능한 목적지, 하늘색이면 이미 뭔가 연결된 상태입니다. 고리가
없는 노브(PHASE, RAND, BLEND, DRIVE 등)는 아직 매트릭스 목적지가 없는 것들입니다.
고리는 드래그 중일 때만 마우스를 받으므로 평소에는 노브 조작을 가리지 않습니다.

같은 소스를 같은 노브에 두 번 놓아도 슬롯이 하나 더 생기지 않고, 8슬롯이 모두 차면
드롭이 조용히 무시됩니다(기존 연결을 덮어쓰지 않습니다).

매트릭스 전용 페이지는 아직 없어서, 세밀한 양 조절은 호스트의 기본 파라미터 패널에서
합니다(`Mod Matrix` 그룹의 Slot 1–8). MATRIX 페이지를 그리면 그 UI로 옮겨갑니다.

호스트가 매트릭스 슬롯을 직접 오토메이션하면 고리 색은 다음 드롭 전까지 갱신되지
않습니다. 창을 열 때는 저장된 라우팅을 읽어 제대로 켜집니다.
